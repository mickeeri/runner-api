Knock.setup do |config|
  ...
  ## ResourceOwner handle attribute
  ## ---------------------
  ##
  ...
  ##
  ## Default:
  config.current_user_from_handle = -> (handle) { ResourceOwner.find_by! Knock.handle_attr => handle }

  ## Current user retrieval when validating token
  ## --------------------------------------------
  ##
  ## This is how you can tell Knock how to retrieve the current_user.
  ## By default, it assumes you have a model called `ResourceOwner` and that
  ## the user_id is stored in the 'sub' claim.
  ##
  ## You must raise ActiveRecord::RecordNotFound if the resource cannot be retrieved.
  ##
  ## Default:
  config.current_user_from_token = -> (claims) { ResourceOwner.find claims['sub'] }
